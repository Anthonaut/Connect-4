...
def check_next_elem(board, row, col, row_dir, col_dir):
    next_row = row + row_dir
    next_col = col + col_dir
    if 0 <= next_row <= 5 and 0 <= next_col <= 6:
        if board[row][col] == board[next_row][next_col] and board[row][col] != EMPTY:
            return True
    return False
...
def check_column(board, col):
    ...
    while row_num < len(board):
        if check_next_elem(board, row_num, col, 1, 0):
            match += 1
            if match == 3:
                return True
        else:
...
def check_row(board, row):
    col_num = 0
    match = 0
    while col_num < len(board[0]):
        if check_next_elem(board, row, col_num, 0, 1):
            match += 1
            if match == 3:
                return True
        else:
...
def check_diagonal(board, row_dir, start_row, start_col):
    row = start_row
    col = start_col
    match = 0
    while (0 <= row < len(board)) and (0 <= col < len(board[0])):
        if check_next_elem(board, row, col, row_dir, 1):
            match += 1
            if match == 3:
                return True
        else:
...
def check_diagonal_NE(board):
    start_row = 0
    start_col = 0
    while start_row < len(board):
        if check_diagonal(board, -1, start_row, start_col):
            return True
        start_row += 1
...
def has_connect_four(board):
    ...
    if check_vertical(board):
        return True
    if check_diagonal_NE(board):
        return True
    if check_diagonal_SE(board):
        return True
    return False
...
def get_input(board, player):
    col_choice = int(input('Please choose a column: '))
    while not 0 <= col_choice < len(board[0]):
        col_choice = int(input('Please choose a column: '))
    return col_choice
